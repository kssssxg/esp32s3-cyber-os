...
#include <string.h>
// SD.h 不在这里包含，避免引入Arduino.h导致C++标准库冲突
// 存档功能在nes_bridge.cpp中处理
...
volatile int get_pad0_calls = 0;
volatile int mem_read_4016_count = 0;
volatile uint32_t last_cpu_pc = 0;
volatile uint8_t last_ram_0000 = 0;
volatile uint8_t last_ram_0001 = 0;

// 桥接函数: 由nes_bridge.cpp实现，返回当前手柄状态
...
volatile uint8_t controller_raw = 0;

// get_pad0: 游戏通过$4016读取手柄，但我们已在NMI后直接注入$06FC
// 这里返回原始位数据(可能被游戏的ROL反转搞乱)，不影响注入方案
__attribute__((noinline)) uint8_t get_pad0(void)
{
    uint8_t raw = nes_bridge_get_pad0_state();
    uint8_t cnt = pad0_readcount;
    if (cnt >= 16) return 0;
    uint8_t bit = ((raw >> cnt) & 1);
    pad0_readcount++;
    get_pad0_calls++;
    controller_raw = raw;
    return bit;
...
// 每帧开始时重置shift register计数器，确保手柄读取正确
...
uint8_t get_pad1(void)
{
    uint8_t value = nes_bridge_get_pad1_state();
    return (((value >> (7 - pad1_readcount++)) & 1));
}
...
__attribute__((noinline, optimize("O0"))) static uint8_t mem_readbyte(uint32_t address)
{
    nes6502_memread *mr;
    ...
    {
        mem_read_4016_count++;
        uint8_t val = get_pad0();
        last_cpu_pc = cpu.pc_reg;
        last_ram_0000 = controller_raw;
        return val;
    }
...
void mmc_bankvrom(int size, uint32_t address, int bank)
{
    ...
    default:
        // log_d("???\n"); // debug message, removed to avoid dependency
        true;
...
void nes_renderframe(bool draw_flag)
{
    ...
    while (262 != NESmachine->scanline)
    {
        ...
        // scanline 241: NMI handler已执行完毕，游戏已写入$06FC
        // 直接覆盖为我们的按键状态，绕过游戏的$4016移位寄存器读取和ROL反转
        if (NESmachine->scanline == 241 && ram) {
            uint8_t pad = nes_bridge_get_pad0_state();
            ram[0x06FC] = pad;
            ram[0x06FD] = 0;
        }
...
// 存档功能需要SD库，暂时禁用以避免Arduino.h引入C++标准库冲突
// 如需存档功能，在nes_bridge.cpp中实现
#ifdef NES_SAVE_STATE_ENABLED
